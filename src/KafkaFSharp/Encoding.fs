...
let PAYLOAD_SIZE_BYTE_INDEX = 0
let MAGIC_BYTE_INDEX = 4
let COMPRESSION_BYTE_INDEX = 5
let CHECKSUM_START_BYTE_INDEX = 6
let PAYLOAD_START_BYTE_INDEX = 10
...
let encode_msg (msg : Message) : byte[] =
  ...
  let payload_size = (msg.payload_size |> BitConverter.GetBytes)
  Array.Copy(payload_size, 0, msg_arr, PAYLOAD_SIZE_BYTE_INDEX, 4)
  msg_arr.[MAGIC_BYTE_INDEX] <- msg.magic
  msg_arr.[COMPRESSION_BYTE_INDEX] <- msg.compression
  Array.Copy(msg.checksum, 0, msg_arr, CHECKSUM_START_BYTE_INDEX, 4)
  Array.Copy(msg.payload, 0, msg_arr, PAYLOAD_START_BYTE_INDEX, msg.payload.Length)
  msg_arr
...
let decode_msg (decoded_msg : byte[]) : Message =
  ...
  let payload = decoded_msg.[PAYLOAD_START_BYTE_INDEX..]
  ...
  let crc32_provided = BitConverter.ToUInt32(decoded_msg, CHECKSUM_START_BYTE_INDEX)
...
//  let decompressed = decompress_data compression_type decoded_msg.[PAYLOAD_START_BYTE_INDEX..]

  { magic = decoded_msg.[MAGIC_BYTE_INDEX]
    compression = decoded_msg.[COMPRESSION_BYTE_INDEX]
    payload = payload//decompressed
    checksum = Array.sub decoded_msg CHECKSUM_START_BYTE_INDEX 4
    offset = 0UL
