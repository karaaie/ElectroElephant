...
let private deserialize_partition_datas (stream : MemoryStream) =
  let num = stream.read_int32<ArraySize> ()
  [for i in 1..num do yield deserialize_partition_data stream]

let private deserialize_topic_data (stream : MemoryStream) =
  { topic_name = stream.read_str<StringSize> ()
    partition_fetch_data = deserialize_partition_datas stream}

let private deserialize_topic_datas (stream : MemoryStream) =
  let num = stream.read_int32<ArraySize> ()
  [for i in 1..num do yield deserialize_topic_data stream]
...
let deserialize (stream : MemoryStream) =
  { replica_id = stream.read_int32<ReplicaId> ()
    topic_offset_data = deserialize_topic_datas stream}