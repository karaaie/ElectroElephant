...
let private send_request<'RequestType> (req_t : RequestTypes) (socket : Socket)
    : Async<Response> = 
  //B. 
  async {
    ...
    use netstream = new NetworkStream(socket)

    let rec send (load : byte[]) = async { 
       do! netstream.AsyncWrite( load, 0, load.Length)
     }
    
    //send expected msg size to broker
    do! send(payload_size)
    Logger.info logger "payload size sent"
    //send the actual msg
    do! send(payload_state.payload)
    Logger.info logger "actual payload sent"

    /// reads the expected amount of bytes in to the given buffer
    /// the number returned is the amount of bytes read.
    let rec read (buffer : byte[]) (expected_size : int) (buffer_offset : int) = async {
       let! read_bytes = Async.AwaitTask (netstream.ReadAsync(buffer,buffer_offset, expected_size))
       if expected_size < read_bytes then do! read buffer (expected_size - read_bytes) (buffer_offset+read_bytes)
    }

    let msg_size_buffer = Array.zeroCreate sizeof<MessageSize>
    //read the msg size from the buffer
    do! read msg_size_buffer sizeof<MessageSize> 0

    let msg_size = BitConverter.ToInt32(msg_size_state.payload |> correct_endianess, 0)

    LogLine.info "msg size response received"
    |> LogLine.setData "size" msg_size |> Logger.log logger

    let msg_buffer = Array.zeroCreate msg_size 
    do! read msg_buffer msg_size 0

    Logger.info logger "msg response received"
    use memStream = new MemoryStream(msg_buffer)
    return (Response.deserialize (api_key req_t) memStream)
    }
...
let private unwrapp_metadata_response resp : MetadataResponse = 
  ...
  | wrong -> 
    ...
    |> Logger.log logger
    failwith "got unexpected return type!"

let do_metadata_request (hostname : string) (port : int) 
    (topics : string list option) (callback : MetadataResponse -> unit) : Async<MetadataResponse> = 
  async {
    ...
    let! response = send_request<MetadataRequest> meta_req socket.Client

    return (unwrapp_metadata_response response)
  }