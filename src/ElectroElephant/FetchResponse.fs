...
let private serialize_topic_message (stream : MemoryStream) topic_message = 
  ...
  topic_message.partition_messages |> List.iter (serialize_partition_message stream)
...
let private deserialize_topic_message (stream : MemoryStream) = 
  { topic_name = stream.read_str<StringSize>()
    partition_messages = read_array<FetchedPartitionMessage> stream deserialize_partition_message }
...
let deserialize (stream : MemoryStream) = 
  { topic_messages = read_array<FetchedTopicMessage> stream deserialize_topic_message }
